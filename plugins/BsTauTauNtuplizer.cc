...
Float_t BsTauTauNtuplizer::getMaxDoca2(std::vector<RefCountedKinematicParticle> &kinParticles){
...
Float_t BsTauTauNtuplizer::getMinDoca2(std::vector<RefCountedKinematicParticle> &kinParticles) {
  ...
  //////////////////////////////
   std::cout << "Starts to build tau candidate out of " << numOfch << " pion candidates" << std::endl;
	...
	for(int kkk = jjj+1; kkk < numOfch; kkk ++){
	  ...
	  for(int itrk = 0; itrk < numOfch;  itrk++){
	    ...
	    VertexDistance3D a3d_pf;  
            ...
            std::pair<bool,Measurement1D> cur3DIP_pf = BsTauTauNtuplizer::absoluteImpactParameter(tsos_pf, bc_vertex, a3d_pf);
	...
	taucand _cand_ = {
	    ...
	    (Int_t) tau_charge,
	    //(Int_t) tau_charge1, // change
	    //(Int_t) tau_charge2, // change
	    //(Int_t) tau_charge3, // change
	    (Bool_t) isRight,
	    (Bool_t) isRight1,
	    (Bool_t) isRight2,
	    (Bool_t) isRight3,
...
//      }
	...
	rhomass.push_back(tlv_rho.M());
      ...
      Int_t pi1_charge = pf1.charge(); //[ic]; //  pf1.charge();
      Int_t pi2_charge = pf2.charge(); //[ic]; ; //pf2.charge();
      Int_t pi3_charge = pf3.charge(); //[ic]; ; //pf3.charge();

      Float_t precut_dz1 = pf1.vz() - closestVertex.position().z();
      Float_t precut_dz2 = pf2.vz() - closestVertex.position().z();
      Float_t precut_dz3 = pf3.vz() - closestVertex.position().z();


      nBranches_->BsTauTau_tau_pi1_charge.push_back(pi1_charge); // change
      nBranches_->BsTauTau_tau_pi2_charge.push_back(pi2_charge);
      nBranches_->BsTauTau_tau_pi3_charge.push_back(pi3_charge);

      nBranches_->BsTauTau_tau_pi1_dz.push_back(precut_dz1); // change
      nBranches_->BsTauTau_tau_pi2_dz.push_back(precut_dz2);
      nBranches_->BsTauTau_tau_pi3_dz.push_back(precut_dz3);
	...
	//	std::cout << "rho masses size = " << rhomass.size() << std::endl;
      ...
      nBranches_->BsTauTau_tau_lip.push_back(cands[ic].cand_tau_lip);
      nBranches_->BsTauTau_tau_lips.push_back(cands[ic].cand_tau_lips);
      nBranches_->BsTauTau_tau_pvip.push_back(cands[ic].cand_tau_pvip);
      nBranches_->BsTauTau_tau_pvips.push_back(cands[ic].cand_tau_pvips);
      nBranches_->BsTauTau_tau_fl3d.push_back(cands[ic].cand_tau_fl3d);
      nBranches_->BsTauTau_tau_fls3d.push_back(cands[ic].cand_tau_fls3d);
      nBranches_->BsTauTau_tau_alpha.push_back(cands[ic].cand_tau_alpha);
      ...
      nBranches_->BsTauTau_B_lip.push_back(cands[ic].cand_b_lip);
      nBranches_->BsTauTau_B_lips.push_back(cands[ic].cand_b_lips);
      nBranches_->BsTauTau_B_pvip.push_back(cands[ic].cand_b_pvip);
      nBranches_->BsTauTau_B_pvips.push_back(cands[ic].cand_b_pvips);
      nBranches_->BsTauTau_B_fls3d.push_back(cands[ic].cand_b_fls3d);
      nBranches_->BsTauTau_B_fl3d.push_back(cands[ic].cand_b_fl3d);
      nBranches_->BsTauTau_B_alpha.push_back(cands[ic].cand_b_alpha);
      ...
      nBranches_->BsTauTau_B_maxdoca.push_back(getMaxDoca(allParticles4doc));
      nBranches_->BsTauTau_B_mindoca.push_back(getMinDoca(allParticles4doc));

      nBranches_->BsTauTau_muonpion_maxdoca.push_back(getMaxDoca2(allParticles4doc)); //change
      nBranches_->BsTauTau_muonpion_mindoca.push_back(getMinDoca2(allParticles4doc)); //change


      //deltaR muon pion - change

      Float_t muon_dr1 = reco::deltaR(tlv_pion1.Eta(), tlv_pion1.Phi(), muoncollection[0].eta(), muoncollection[0].phi()); // change
      Float_t muon_dr2 = reco::deltaR(tlv_pion2.Eta(), tlv_pion2.Phi(), muoncollection[0].eta(), muoncollection[0].phi()); // change
      Float_t muon_dr3 = reco::deltaR(tlv_pion3.Eta(), tlv_pion3.Phi(), muoncollection[0].eta(), muoncollection[0].phi()); // change
    
      nBranches_->BsTauTau_tau_muon_dr1.push_back(muon_dr1); // change
      nBranches_->BsTauTau_tau_muon_dr2.push_back(muon_dr2); // change
      nBranches_->BsTauTau_tau_muon_dr3.push_back(muon_dr3); // change
      ...
      nBranches_->BsTauTau_mu1_dbiso.push_back(MuonPFIso(muoncollection[0]));
