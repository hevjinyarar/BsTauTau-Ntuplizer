...
#include "../interface/AllParticlesNtuplizer.h" //change
...

{
  ...
  runFlags["doAllParticles"] = iConfig.getParameter<bool>("doAllParticles"); //change
...
//change
  if (runFlags["doAllParticles"]) {
    //std::vector<edm::EDGetTokenT<pat::PackedCandidateCollection>> allpTokens;
    //allpTokens.push_back( packedpfcandidatesToken_ );
//    allpTokens.push_back( allparticleToken_ );

    //edm::EDGetTokenT<pat::PackedCandidateCollection> packedpfcandidatesToken;

    //nTuplizers_["allParticles"] = new AllParticlesNtuplizer( allpTokens, nBranches_, runFlags );
    nTuplizers_["allParticles"] = new AllParticlesNtuplizer( packedpfcandidatesToken_, nBranches_, runFlags );
  }
